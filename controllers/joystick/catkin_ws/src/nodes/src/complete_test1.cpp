...
void positionSensorCallback1(const std_msgs::Float64::ConstPtr& value)
{
	...
	//ROS_INFO("position sensor sent value rad  %f", value->data);
...
/* void testCallback(const nodes::LegCommand::ConstPtr& msg){
	ROS_INFO("testCallback");
} */

// Callback function for reading pwm command
void pwmCallback(const nodes::LegCommand::ConstPtr& msg)
{
	 pwm_msg.pwm_duty_hip = (int8_t) msg->pwm_duty_hip;
	 
	 ROS_INFO("PWM value: %d", msg->pwm_duty_hip);
}
...
int main(int argc, char **argv)
{
  ...
  //ros::Subscriber test_sub = n.subscribe("Test_topic", 1000, testCallback);
  //ros::spin();
  ...
  // Creates a client for the set_torque service
  ros::ServiceClient set_torque_client1 = n.serviceClient<nodes::motor_set_torque>(model_name+"/servo1_rot/set_torque");
  nodes::motor_set_torque set_torque_srv1; // Here we instantiate an autogenerated service class.

 /*  set_torque_srv1.request.torque =  MAXTORQUE * pwmHip1*0.001;
...
*/
  ...
  if (time_step_client.call(time_step_srv) && time_step_srv.response.success == 2)
    ...
    while (ros::ok())
    {
        ...
        i66++;
    //*********************************service request for torque (shouldn't this be placed in the Webots ROS node?)***********************************************
		
		set_torque_srv1.request.torque = MAXTORQUE * pwm_msg.pwm_duty_hip*0.01;
